...
#Transfer Drivers between Boot Images in MECM
$from = 
$to = 
...
7. Assign letter=k (or any available letter you want)
8. Format quick fs=FAT32
9. Create partition msr size=128
10. Create partition primary
11. Assign letter=c (at this step if the C letter is not available, check if you have USB key mounted)
12. Format quick fs=NTFS
13. Exit
...
Function ExtractIcon {
    ...
    dir $folder *.exe -ea 0 -rec |
      ForEach-Object { 
        ...
        [System.Drawing.Icon]::ExtractAssociatedIcon($_.FullName).ToBitmap().Save("$folder\$BaseName.jpg")
    }
}
